...
contract("TokenDistributor", (accounts) => {
    ...
    before(async () => {
        ...
        assert.exists(token.address, "DummyToken was not deployed correctly.");
        assert.exists(tokenDistributor, "TokenDistributor was not deployed correctly.");

        const targetToken = await tokenDistributor.targetToken();

        assert.strictEqual(
            targetToken,
            token.address,
            "The targetToken was not set correctly in TokenDistributor."
        );
